...
char *get_auth_url(const char *token) {
  ...
  sprintf(petition, "%s?request_token=%s&redirect_uri=%s", URL_AUTH, token,
          REDIRECT_URI);
...
int pocket_init() { return curl_global_init(CURL_GLOBAL_ALL); }

void pocket_clean() { curl_global_cleanup(); }
...
char *pocket_auth(const char *key) { return NULL; }
